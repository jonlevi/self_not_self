...
# Manuscript
LATEX code and figures for the manuscript can be found in the `paper/` directory. Each figure in our paper can be reproduced by running the appropriate jupyter notebook in the `notebooks` folder. All dependencies used are publically available and can be pip installed per usual, although if you prefer we have an environment yml file (peptidome_environment.yml) provided that can be used to set up a conda environment.
